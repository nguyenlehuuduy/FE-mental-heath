...
import { FilterIcon, MessageIcon, NotifyIcon, SearchIcon } from "../../icons";
import NotifyPopup from "../NotifyPopup";
import { MyselfForCard, getLoginAccount } from "@/service/accountService";
...
export default async function Header({ profile }: { profile: MyselfForCard }) {
  const userInfo = await getLoginAccount();
  const listNotifi = await getListNotification();
    ...
    <header className="border-b w-full z-10 bg-white px-4">
      <div className="max-w-[1440px] h-[60px] mx-auto flex justify-evenly items-center py-3">
        ...
        <div className="flex flex-row gap-3 max-w-[300px] w-full h-full justify-end items-center">
          ...
          <NotifyPopup listNotifi={listNotifi ?? []}>
          ...
          <ProfilePopup>
            <div className="relative cursor-pointer">
              <AvatarAccount
                filePath={userInfo?.avata}
                name={userInfo?.full_name ?? "D"}
              />
            </div>
          </ProfilePopup>
        </div>
